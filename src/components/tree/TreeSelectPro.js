...
export default class TreeSelectPro extends React.Component {
    ...
    componentDidMount = () => {
        const {setData,url,values,ajax,dataSourceValue} = this.props;
        if (url) {
            ajax(url, {...values}, (data) => {
                this.setState({
                    dataSource: setData(data).map((datas,i)=> {
                        datas.key = datas[dataSourceValue]
                        return datas;
                    })
                })
            })
    ...
    render() {
        const {treeData, width, value, config, multiple, clotheLang,url} = this.props
        const {dataSource} = this.state

        let _treeData = url?dataSource:treeData
        ...
        return (
            <TreeSelect
                ...
                {...config}
            >
                {this.renderTreeNodes(_treeData)}
            </TreeSelect>
        );
    }
}
